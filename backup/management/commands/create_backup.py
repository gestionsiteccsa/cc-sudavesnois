...
class Command(BaseCommand):
    help = (
        "Crée une sauvegarde automatique (BDD + media) et envoie une notification email"
    )

    def handle(self, *args, **options):
        self.stdout.write(
            self.style.NOTICE("Démarrage de la sauvegarde automatique...")
        )
        ...
        try:
            ...
            if deleted:
                self.stdout.write(
                    self.style.NOTICE(
                        f'Anciens backups supprimés : {", ".join(deleted)}'
                    )
                )
            ...
            if settings.notification_email:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Notification envoyée à {settings.notification_email}"
                    )
                )

            self.stdout.write(
                self.style.SUCCESS("Sauvegarde automatique terminée avec succès !")
            )
